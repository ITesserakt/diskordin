...
object FrameTransformer : Transformer<Frame, WebSocketEvent> {
    override fun transform(context: Frame): WebSocketEvent = when (context) {
        is Frame.Close -> context.readReason()?.toEvent() ?: WebSocketEvent.ConnectionClosed(1000, "Normal close")
    ...
    private fun CloseReason.toEvent() = WebSocketEvent.ConnectionClosed(code, message)
